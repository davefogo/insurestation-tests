### Third-party Rater Tests 

This repository contains the UI tests that go through insurestation. It uses the `chromedriver` NPM package that wraps the ChromeDriver for you. This service does not require a Selenium server, but uses ChromeDriver to communicate with the browser directly.
...
`JDK 1.8:` It is optional, install JDK 1.8+ and make sure class path is set properly. JAVA is required to start `Selenium Server` on your local environment nothing else.
...
The default option for Grunt run is `webdriver:test-local`. But you can use `test-local` based on your requirements.
